...
class TrainingCollator(object):  # Visual Question Answering Collator
    ...
    def __call__(self, batch):
        ...
        texts = [item["text"] for item in batch if item is not None]
        answers = [item["answer"] for item in batch if item is not None]
...
class TestingCollator(object):  # https://huggingface.co/datasets/Lin-Chen/MMStar
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer
    
    def __call__(self, batch):
        images = [item["image"] for item in batch if item is not None]
        questions = [item["text"] for item in batch if item is not None]
        answers = [item["answer"] for item in batch if item is not None]
