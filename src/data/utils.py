...
from src.data.collators import TrainingCollator, TestingCollator
...
def get_split_loaders(dataset, tokenizer, val_size=0.1, batch_size=128, num_workers=4):
    n = len(dataset)
    v = int(val_size * n)
    print('train size:', n-v, ' val size:', v)
    train_data, val_data = random_split(dataset, [n - v, v])
    tl = DataLoader(
        train_data,
        ...
        shuffle=True, 
        pin_memory=True, 
        collate_fn=TrainingCollator(tokenizer, max_length=dataset.max_length)
    )
    vl = DataLoader(
        val_data, 
        batch_size=batch_size, 
...
def get_test_loader(dataset, tokenizer, batch_size=128, num_workers=4):
    ...
    return DataLoader(
        ...
        num_workers=num_workers, 
        pin_memory=True, 
        collate_fn=TestingCollator(tokenizer)
    )